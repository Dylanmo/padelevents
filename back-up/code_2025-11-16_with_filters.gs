...
function doGet(e) {
  ...
  if (action === 'ics') {
    ...
    return ContentService.createTextOutput(icsText)
...
function parseLevelRange(s) {
  const m = String(s || '').match(/([0-9]+(?:\.[0-9]+)?)\s*[–-]\s*([0-9]+(?:\.[0-9]+)?)/);
  return m
...
function previewByClubs(payload) {
  ...
  const filtered = raw
    ...
    .filter((r) =>
      clubs && clubs.length
        ? clubs.includes(String(r.club || '').trim())
        : true
    )
    .filter((r) => {
  ...
  const out = (limit ? filtered.slice(0, limit) : filtered).map((r) => {
    ...
    const eventCategory = r.category || r.event_category || r.type || r.format || '';
    ...
    const available = asInt(r.spots_available || r.available || r.slots_open || r.open);
...
function uidForEvent_(ev) {
  ...
  const hash = Utilities.base64Encode(Utilities.computeDigest(Utilities.DigestAlgorithm.SHA_256, base))
                .replace(/[^A-Za-z0-9]/g,'').slice(0,24);
  return hash + '@padel';
}
function icsEscape_(s){ return String(s||'').replace(/([,;])/g,'\\$1').replace(/\n/g,'\\n'); }
function dtLocal_(iso, tz){ return Utilities.formatDate(new Date(iso), tz, "yyyyMMdd'T'HHmmss"); }
